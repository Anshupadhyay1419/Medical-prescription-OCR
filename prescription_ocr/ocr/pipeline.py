...
End-to-end orchestration for one image.
    ...
    detect -> recognise -> [Point D restructure] -> [Point B correct] -> clean
...
from prescription_ocr.config import USE_CORRECTOR, USE_RERANKER, USE_RESTRUCTURER
...
def apply_llm_stages(lines_with_boxes,
    ...
    if use_restructurer:
        print("[3/5] Point D: LLM document restructuring...")
        t0 = time.time()
    ...
    if use_corrector:
        print("[4/5] Point B: LLM line-level correction...")
        t0 = time.time()
...
def run_pipeline(img_path,
                 ...
                 use_corrector=USE_CORRECTOR):
    """
    Recognise one image and write whichever arms were requested.

    output_path      full pipeline result (None skips the LLM stages entirely)
    ...
    print(f"Config: reranker={use_reranker}, restructurer={use_restructurer}, "
          f"corrector={use_corrector}")
    print()
    ...
    if output_path:
        lines = apply_llm_stages(lines_with_boxes,
                                 use_restructurer=use_restructurer,
                                 use_corrector=use_corrector)
        print("[5/5] Deterministic cleanup...")
